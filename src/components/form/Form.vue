<template>
  <div>
    <form v-if="!isSubmitted" @submit.prevent="submit" novalidate>
      ...
      <div class="form-group">
        <label for="cell">{{ $t('form.cell') }} *</label>
        <input type="cell" class="form-control" id="cell" v-model.lazy.trim="form.cell" @blur="onFieldBlur('cell')" v-bind:class="getFieldClasses('cell')">
        <div v-if="isErrorField('cell')" class="invalid-feedback">{{ $t('error.fieldInvalid', { field: $t('form.cell') }) }}</div>
      </div>
      <div class="form-group">
